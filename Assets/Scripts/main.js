...
function Start () {
	pieceType = 2; putPiece(Vector2(3,4));
	pieceType = 1; putPiece(Vector2(3,3));
	pieceType = 1; putPiece(Vector2(4,4));
	pieceType = 2; putPiece(Vector2(4,3));
}
...
function putPiece(key:Vector2) {
	...
	if (changeFlag || initialFlag) {
		...
		var position = Vector3(key.x -4.0f + 0.5f, 1, key.y -4.0f + 0.5f);
		if (pieceType == 1) {
		...
		if (!checkEnablePut() && !initialFlag) {
			...
			if (!checkEnablePut() && !initialFlag) {
				...
				while (!Input.GetButtonDown('Fire1') || Input.touches.Length > 0) yield;
...
function OnGUI() {
	...
	var rect_piece : Rect = Rect(0, 30, Screen.width, Screen.height);
